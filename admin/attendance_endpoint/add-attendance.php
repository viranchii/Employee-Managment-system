...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['qr_code'])) {
        ...
        $selectStmt = $dbh->prepare("SELECT tbl_student_id FROM tbl_student WHERE generated_code = :generated_code");
        ...
        try {
            $stmt = $dbh->prepare("INSERT INTO tbl_attendance (tbl_student_id, time_in) VALUES (:tbl_student_id, :time_in)");
            
            $stmt->bindParam(":tbl_student_id", $studentID, PDO::PARAM_STR); 
            $stmt->bindParam(":time_in", $timeIn, PDO::PARAM_STR); 

            $stmt->execute();
            header("Location: http://localhost/ETMS-MAIN/admin/attendance_home.php");
