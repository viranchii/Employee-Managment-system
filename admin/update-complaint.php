...
if (strlen( $_SESSION['etmsaid']) == 0) {
  ...
  if (isset($_POST['update'])) {
    ...
    $status = $_POST['status'];
    $remark = $_POST['remark'];
    $add = "insert into complaintremark(complaintNumber,status,remark) values('$complaintnumber','$status','$remark')";
    $query = $dbh->prepare($add);
    $query->execute();

    $sql = "update tblcomplaints set status='$status' where complaintNumber='$complaintnumber'";
